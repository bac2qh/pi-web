...
- **让重要会话随手可达**：可以跨项目固定会话，使用滚动十天窗口的 Recent 列表，在 Lineage 中浏览所选会话的完整可用原生家族，也可以隐藏不需要的 Fork 子树，并用“显示隐藏会话”临时查看。
- **放心试不同方向**：可以从历史消息处编辑、在某条旧提示之前 Fork、运行 `/clone` 复制当前完整活动分支，或用 `/side` 在源会话继续运行时打开一个持久的调查会话。
...
- **侧边栏状态**：固定会话和显式隐藏标记通过 pi agent 目录下的 `pi-web-sidebar.json` 在多个 pi-web 客户端之间共享。分区顺序是 Pinned、Recent、Lineage、Project、Explorer；Lineage 默认展开，Project 默认折叠，两者的分区、分支和滚动状态各自独立且只在当前页面加载期间保留。Lineage 根据完整可用列表中的原生 `parentSession` 关系派生。隐藏只影响显示，不会移动或重写会话 JSONL、改变固定状态，也不会停止正在运行的会话。Pi Web 不提供永久删除会话的功能。
...
- **分支操作**：“Edit from here” 在当前会话文件内创建分支；Fork 从选中的历史提示之前创建子 `.jsonl` 并打开它；`/clone` 把直到当前位置的完整活动分支创建为普通子会话，pi-web 会刷新侧边栏，但仍停留在源会话。精确的 `/side` 会截取最新、结构完整的已持久化前缀，创建并选中一个具名的持久子会话，同时保持源会话的当前轮次不变。
- **Side 会话**：模型仍可使用继承上下文，但普通 Side 聊天记录会把它隐藏。可以用 **Return to parent** 或会话树返回；Side 子会话会一直保留，除非你主动 Hide。Side 会话不能 Fork、Clone 或再创建 Side；已知的 subagent 和 Start/Orchestrate 启动能力会被移除，但普通检查和工作区工具仍可使用。**Full history** 会有意显示完整原生会话文件，包括继承上下文。
- **Clone/Side 的宿主边界**：`/clone` 和 `/side` 使用 Pi 原生的会话文件提取和父子关系格式，但都不会替换正在运行的源 Web 会话，也不会触发 TUI 的 fork/clone 扩展生命周期事件。
...
components/
  ...
  SessionSidebar.tsx  # Pinned/Recent/Lineage/Project 分区和 Explorer
...
lib/
  ...
  sidebar-session-state.ts    # 固定/隐藏/Recent/Lineage/Project 树的纯派生逻辑
