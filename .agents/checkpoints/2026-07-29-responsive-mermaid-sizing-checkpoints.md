...
**Work and outcome:** Completed shrink-only responsive Mermaid sizing. Every successful returned SVG with a structurally valid, finite, positive viewBox is finalized in a detached browser template before React state publication with root `width="100%"`, inline `width: 100%`, natural viewBox pixel `max-width`, and `height: auto`; small diagrams stop at natural width, oversized diagrams follow panel content width, and aspect ratio remains viewBox-driven. Strict SVG-number and exact SVG-whitespace parsing preserve the original Mermaid string for missing or unusable geometry. Finalization is idempotent and retains viewBox, accessibility, identifiers, descendants, unrelated root attributes/styles, and generated content. The visible-DOM layout effect/ref/data mutation and conflicting Mermaid CSS maximum were removed while block display and defensive overflow remain. Streaming, Preview/Source ownership, completion folding, theme/Transcript regeneration, reload/history, bounded invalid Preview, strict security, suppressed error rendering, dynamic import, serialized queue recovery, stale-result cancellation, diagnostics, and render keys are unchanged. No dependency, telemetry, API, cache, folding redesign, wiki, or durable-memory change was added.

**Validation / evidence:** VC-001 through VC-006 pass. Final tracked validation is 137/137 Node tests, 14/14 focused Mermaid tests, `/Users/xin/Documents/repos/pi-web/node_modules/.bin/tsc --noEmit`, `npm run lint`, and `git diff --check`; `next build` was never run. Privacy-safe Chromium evidence under `/Users/xin/Documents/repos/pi-web/.agents/worktrees/.runtime/2026-07-29-responsive-mermaid-sizing/` records natural-width and shrink-to-panel geometry across flowchart, sequence, class, and pie, preserved ratios and parser integrity/idempotence, missing-viewBox overflow fallback, Source-to-Preview, theme, non-default Transcript size, streaming completion, remount, invalid Preview/Source, and panel narrowing/widening. `component-report.json` records 42 anonymous insertions with all 37 valid instances final at first observation and zero post-insertion sizing mutations; `live-report.json` records the real AppShell/ChatWindow/useAgentSession fake-SSE `message_end` to `agent_end` folding, process-parent remount, late entries, reload/history, equal process/final settlement viewBoxes, all 29 valid live/reload insertions final, zero post-insertion sizing mutations, and zero browser errors or warnings. Two privacy-safe screenshots are retained; all source/session-bearing harnesses and logs were removed and isolated port 30143 was stopped. Independent review found the non-SVG `\s` fallback defect; the parent fixed it, final tests passed, and fresh independent review confirmed no remaining blocker.

**Departures from approved obligations:** None. CSSOM may canonically serialize long decimal style values, but the application policy does not round them and measured geometry remains within subpixel tolerance of the natural viewBox maximum. Browser evidence is summarized rather than replay-scripted because retaining synthetic source/session harnesses would violate the approved privacy boundary; the final parser-only tightening leaves every valid browser fixture path unchanged and has direct focused coverage. All approved obligations are complete.

**Implementation commit:** `037407e13886f3d0e818b53570f0b715ee9457d0` (`feat: make Mermaid previews shrink responsively`).
