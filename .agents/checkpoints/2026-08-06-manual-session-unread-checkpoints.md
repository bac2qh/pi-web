...
**Plan section:** Final implementation, review, Validation Contract VC-001 through VC-006, and closeout readiness for the full approved plan.

**Work and outcome:** The existing browser-local blue-dot lifecycle now includes an accessible dynamic `Mark unread` / `Mark read` command in every Pinned, Recent, and Project row. One shared set synchronizes duplicate presentations, explicit row opens clear unread even on selected-row re-click, running status clears and visually supersedes unread, and unselected completion restores it. Guarded storage, stale pruning, and pure transitions are extracted and tested. Metadata spacing and fork-control offsets preserve the fixed row layout. Pin/unpin, Hide/Restore, rename, collapse/expand, navigation, selection, sorting, the strict shared sidebar schema, JSONL, and native ownership remain unchanged. Current-state documentation, durable memory, review handoffs, and privacy-safe evidence are committed.

**Validation / evidence:** Implementation commit `9f603fdaa13e0e3a9a914d79a3d86f5c26f22092`. Final automated evidence records 726/726 Node tests passing, TypeScript passing, lint passing, and `git diff --check` passing in `.agents/reports/2026-08-06-manual-session-unread/automated-validation.txt`. The final Chromium report passes with zero browser errors and covers mouse, actual Space-key activation, focus visibility, touch input, selected/unselected open lifecycle, reload, malformed/stale storage, running/idle precedence and completion, duplicate presentations, narrow geometry, fixed height, no overflow, and direct collapse/expand, unpin/repin, Hide/Restore, rename, unread-preservation, and selection-preservation checks. Three fresh review rounds ended clean after resolving the existing-control evidence gap and removing contradictory `aria-pressed` toggle semantics from the dynamically named command. No transient validation route, fixture, `.pi-subagents`, dependency cache, generated Next state, staged file, or unrelated worktree change remains.

**Departures from approved obligations:** None. No obligation is incomplete, blocked, waived, superseded, or divergent. The browser running/idle sequence uses synthetic publication through the real page-global status boundary rather than a paid provider/model call; lower-layer production publication is covered by the full suite, so no validation obligation was waived. No screen-reader application was available, but the final ordinary-command semantics remove the identified conflict and native name, keyboard, focus, and touch behavior are directly verified.

**Implementation commit:** `9f603fdaa13e0e3a9a914d79a3d86f5c26f22092`.
