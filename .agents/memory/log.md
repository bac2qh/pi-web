...
- Added dependency-security and local-runtime memory after manual completion of the npm security refresh and checkout-local hybrid cutover.
...
- Added agent-response-file-links memory after separating strict automatic assistant actions from authored links, enforcing exact-cwd/text-only safety, and defining guarded narrow activation with expansion provenance.
...
- Updated browser-local display-preference memory after replacing the artificial `1000px` panel-resize cutoff with split-presentation and real-range eligibility, preserving visible-panel bounds and minima, and widening coarse-pointer targets without changing separator appearance.
