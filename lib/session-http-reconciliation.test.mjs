...
function observation(patch = {}) {
  ...
  return Object.freeze({
    ...
    transport,
    selectedLeafId: patch.selectedLeafId === undefined ? "leaf" : patch.selectedLeafId,
...
test("live and pinned leaf intent transitions reject superseded transcript modes", () => {
  const coordinator = new SessionHttpReconciliation();
  const live = observation({ selectedLeafId: null, leafGeneration: 5 });
  const liveToken = coordinator.begin("transcript", live);
  assert.equal(coordinator.decide(liveToken, live), "accepted", "a displayed live tip is not itself a selected pin");

  const pinned = observation({ transport: live.transport, selectedLeafId: "ancestor", leafGeneration: 6 });
  assert.equal(coordinator.decide(liveToken, pinned), "stale_leaf", "explicit navigation invalidates a queued live-root response");

  const contextToken = coordinator.begin("context", pinned);
  const followingDescendant = observation({ transport: live.transport, selectedLeafId: null, leafGeneration: 7 });
  assert.equal(coordinator.decide(contextToken, followingDescendant), "stale_leaf", "prompt-time pin release invalidates ancestor context");
});
