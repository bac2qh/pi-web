...
const newAgentRoute = await jiti.import("../app/api/agent/new/route.ts");
const { createNewAgentPost } = await jiti.import("./new-agent-route.ts");
const { POST: postAgent } = await jiti.import("../app/api/agent/[id]/route.ts");
const { AgentSessionWrapper, getOrCreateRpcSession, getRpcSession } = await jiti.import("./rpc-manager.ts");
...
function syntheticAgentSessionWrapper(manager, options = {}) {
  ...
  const inner = {
    ...
    agent: { state: {} },
    extensionRunner: {
      emit: options.emitShutdown ?? (async (event) => { (state.shutdownEvents ??= []).push(event); }),
    },
...
test("agent/new adapter exposes only POST and preserves malformed-request validation", async () => {
  const exportNames = Object.getOwnPropertyNames(newAgentRoute)
    .filter((name) => name !== "__esModule")
    .sort();
  assert.deepEqual(exportNames, ["POST"]);
  assert.equal(typeof newAgentRoute.POST, "function");

  const sessionsBefore = [...(globalThis.__piSessions?.entries() ?? [])];
  const startLocksBefore = [...(globalThis.__piStartLocks?.entries() ?? [])];
  const response = await newAgentRoute.POST(new Request("http://localhost/api/agent/new", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: "{}",
  }));
  ...
  assert.deepEqual(await response.json(), { error: "cwd is required" });
  assert.deepEqual([...(globalThis.__piSessions?.entries() ?? [])], sessionsBefore);
  assert.deepEqual([...(globalThis.__piStartLocks?.entries() ?? [])], startLocksBefore);
});

test("failed ensure awaits published shutdown before removing both cache directions", async (t) => {
  let resolveShutdown;
  ...
  const responsePromise = post(newAgentRequest(owner.cwd, { type: "ensure_session" }));
  void responsePromise.then(() => { responseSettled = true; });
  await new Promise((resolve) => setImmediate(resolve));
  assert.equal(responseSettled, false);
  assert.equal(owner.wrapper.isAlive(), false, "shutdown closes owner admission immediately");
  ...
  assert.equal(owner.state.abortCalls, 1);
...
test("failed ensure setup destroys the published owner exactly once", async (t) => {
  ...
  for (const failure of setupFailures) {
    await t.test(failure.name, async (t) => {
      ...
      assert.equal(owner.state.abortCalls, 1);
      assert.deepEqual(owner.state.shutdownEvents, [{ type: "session_shutdown", reason: "quit" }]);
...
test("successful ensure preserves its published owner", async (t) => {
  ...
  assert.deepEqual(await response.json(), { success: true, sessionId: owner.id, data: null });
  assert.equal(owner.destroyCalls, 0);
  assert.equal(owner.state.abortCalls, 0);
  assert.deepEqual(owner.state.shutdownEvents ?? [], []);
  assert.equal(owner.state.disposeCalls, 0);
  assert.equal(owner.wrapper.isAlive(), true);
...
test("new ensure fails closed when startup returns a mismatched native owner ID", async (t) => {
  ...
  assert.equal(synthetic.state.abortCalls, 1);
  assert.deepEqual(synthetic.state.shutdownEvents, [{ type: "session_shutdown", reason: "quit" }]);
...
test("actual ticket POST and same-port session channel stay resumable and HTTP-schedulable", { timeout: 60_000 }, async (t) => {
  ...
  // S6 binds the registry to the serving gateway generation. Seed the synthetic
  // owner only after that generation is active so restart teardown is real.
  const synthetic = syntheticAgentSessionWrapper(manager);
  const wrapper = synthetic.wrapper;
  ...
  await server.close();
  assert.equal(await shutdownSocketClosed, 1012, "semantic owner requests a graceful close before coordinator force");
  assert.equal(ownerRecord.subscribers.size, 0, "owner close releases the channel subscriber");
  ...
  const restartWrapper = getRpcSession(id);
  assert.ok(restartWrapper.getProjectedEventHub());
  assert.notEqual(restartWrapper, wrapper, "fresh server generation recreates the exact native owner");
  const restarted = await open(wsBase, httpBase, restartTicket.body.ticket, { epoch: null, cursor: null });
  await waitFor(() => restarted.frames.at(-1)?.type === "snapshot_end", "restart_snapshot");
  assert.equal(restarted.frames[0]?.outcome, "initial_snapshot");
  ...
  assert.equal(restartWrapper.onDestroyCallbacks.size, 2, "runtime registry and fresh channel owner each install one observer");
  const ownerClose = new Promise((resolve) => restarted.socket.once("close", (code) => resolve(code)));
  restartWrapper.destroy();
  assert.equal(await ownerClose, 1012);
  assert.equal(synthetic.state.abortCalls, 1, "server release settles native work before shutdown dispatch");
