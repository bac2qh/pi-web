...
class FakeClient {
  ...
  subscribeEffects(listener) {
    ...
    if (this.config.effectOnSubscribe) {
      listener(protocol.freezeCanonicalData({ streamEpoch: "epoch", sequence: 1, effect: { type: "notice", level: "info", message: "subscribe" } }));
    }
  ...
  start() {
    ...
    if (this.config.snapshotOnStart) this.publish(this.config.snapshotOnStart);
...
test("optional atomic snapshot consumer observes current and synchronous start snapshot before its associated effect", () => {
  const { registry, created } = harness({ snapshotOnStart: 1, effectOnStart: true });
  const order = [];
  let latestCursor = -1;
  const handle = registry.acquire("session", {
    ownership: "visible",
    onSnapshot(snapshot) { latestCursor = snapshot.cursor; order.push(["snapshot", snapshot.cursor]); },
    onEffect(delivery) { order.push(["effect", delivery.sequence, latestCursor]); },
  });
  assert.deepEqual(order, [["snapshot", 0], ["snapshot", 1], ["effect", 1, 1]]);
  assert.equal(created[0].client.startCalls, 1);
...
test("atomic snapshot consumer invalidation during subscribe rolls back cleanup and stop exactly once", () => {
  const created = [];
  let registry;
  registry = new SessionRegistry({
    createClient() { const client = new FakeClient(); created.push(client); return client; },
  });
  assert.throws(() => registry.acquire("session", {
    ownership: "visible",
    onSnapshot() { registry.dispose(); },
  }), /setup_failed/);
  assert.equal(created[0].snapshotCleanupCalls, 1);
  assert.equal(created[0].effectCleanupCalls, 0);
  assert.equal(created[0].stopCalls, 1);
  assert.doesNotThrow(() => registry.dispose());
  assert.equal(created[0].stopCalls, 1);
});

test("effect-subscribe invalidation owns the returned cleanup, skips start, and stops exactly once", () => {
  for (const throwCleanup of [false, true]) {
    const created = [];
    let registry;
    registry = new SessionRegistry({
      createClient() {
        const client = new FakeClient({ effectOnSubscribe: true, throwEffectCleanup: throwCleanup });
        created.push(client);
        return client;
      },
    });
    assert.throws(() => registry.acquire("session", {
      ownership: "visible",
      onEffect() { registry.dispose(); },
    }), /setup_failed/);
    assert.equal(created[0].startCalls, 0, "an invalidated client never starts");
    assert.equal(created[0].snapshotCleanupCalls, 1);
    assert.equal(created[0].effectCleanupCalls, 1, "the not-yet-owned effect cleanup runs once");
    assert.equal(created[0].snapshotListeners.size, 0);
    assert.equal(created[0].effectListeners.size, 0);
    assert.equal(created[0].stopCalls, 1);
    assert.doesNotThrow(() => registry.dispose());
    assert.equal(created[0].effectCleanupCalls, 1, "the dead entry retains no cleanup reference");
    assert.equal(created[0].stopCalls, 1);
  }
});

test("effect-subscribe nested notifications retain order and omitted options keep ordinary setup", () => {
  const { registry, created } = harness({ effectOnSubscribe: true });
  const seen = [];
  const handle = registry.acquire("session", {
    ownership: "visible",
    onEffect(delivery) {
      ...
      if (delivery.sequence === 1) {
        created[0].client.emitEffect(protocol.freezeCanonicalData({
          streamEpoch: "epoch", sequence: 2,
          effect: { type: "notice", level: "info", message: "nested" },
        }));
      }
    },
  });
  ...
  assert.equal(created[0].client.startCalls, 1);
  handle.release();
  assert.equal(created[0].client.snapshotCleanupCalls, 1);
  assert.equal(created[0].client.effectCleanupCalls, 1);
  assert.equal(created[0].client.stopCalls, 1);

  const ordinary = harness();
  const omitted = ordinary.registry.acquire("session", { ownership: "visible" });
  assert.equal(ordinary.created[0].client.startCalls, 1);
  omitted.release();
  assert.equal(ordinary.created[0].client.snapshotCleanupCalls, 1);
  assert.equal(ordinary.created[0].client.effectCleanupCalls, 1);
  assert.equal(ordinary.created[0].client.stopCalls, 1);
});
