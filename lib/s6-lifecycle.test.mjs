...
test("runtime-owner cleanup starts synchronously and exposes one strict join", async () => {
  const gateway = createPiWebTransportGateway();
  let resolveCleanup;
  const cleanup = new Promise((resolve) => { resolveCleanup = resolve; });
  const reasons = [];
  const registration = gateway.registerRuntimeOwner("rpc", (reason) => {
    reasons.push(reason);
    return cleanup;
  });

  assert.equal(gateway.beginShutdown(), true);
  assert.equal(gateway.isAcceptingOwners(), false);
  assert.equal(registration.token.isCurrent(), false);
  assert.deepEqual(reasons, ["server_shutdown"]);
  let settled = false;
  const waiting = gateway.waitForRuntimeOwnerCleanup().then(() => { settled = true; });
...
test("server shutdown joins pending replacement and active runtime-owner generations", async () => {
  const gateway = createPiWebTransportGateway();
  let resolveRetired;
  let resolveActive;
  const retiredCleanup = new Promise((resolve) => { resolveRetired = resolve; });
  const activeCleanup = new Promise((resolve) => { resolveActive = resolve; });
  const reasons = [];

  const retired = gateway.registerRuntimeOwner("rpc", (reason) => {
    reasons.push(`retired:${reason}`);
    return retiredCleanup;
  });
  assert.equal(retired.unregister(), true);
  gateway.registerRuntimeOwner("rpc", (reason) => {
    reasons.push(`active:${reason}`);
    return activeCleanup;
  });

  gateway.beginShutdown();
  assert.deepEqual(reasons, ["retired:owner_replaced", "active:server_shutdown"]);
  let settled = false;
  const waiting = gateway.waitForRuntimeOwnerCleanup().then(() => { settled = true; });
  resolveActive();
  await Promise.resolve();
  assert.equal(settled, false, "retired generation remains part of the strict join");
  resolveRetired();
  await waiting;
  assert.equal(settled, true);
  gateway.close();
...
test("pending semantic cleanup never delays or resets the absolute network force point", async () => {
  const c = clock();
  const h = await createInjectedShutdownServer(c);
  let resolveCleanup;
  const cleanup = new Promise((resolve) => { resolveCleanup = resolve; });
  const reasons = [];
  h.server.gateway.registerRuntimeOwner("rpc", (reason) => {
    reasons.push(reason);
    return cleanup;
  });
  h.server.gateway.registerChannel("session", () => {}, () => {});
  ...
  assert.equal(webSocket.terminateCalls, 1, "network force remains exactly at 10,000ms");
  assert.equal(raw.destroyCalls, 1, "HTTP force shares the original deadline");
  assert.equal(closeSettled, false, "strict semantic cleanup remains independently pending");
