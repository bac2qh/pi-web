...
test("insertion publishes both fresh replacement edges under one revision and receipt", (t) => withStore(t, async (agentDir) => {
  await mutateSessionDagState(add("setup", 0, "edge-ab"), {
    agentDir,
    availableSessionIds: AVAILABLE,
  });
  const result = await mutateSessionDagState(insert("insert", 1), {
    agentDir,
    availableSessionIds: AVAILABLE,
  });
  assert.equal(result.changed, true);
  assert.equal(result.state.revision, 2);
  assert.deepEqual(result.state.activeEdges, [
    {
      id: "edge-ac",
      formId: "default",
      fromSessionId: "A",
  ...
  assert.equal(result.state.nextEdgeOrder, 3);

  const stored = JSON.parse(await readFile(join(agentDir, SESSION_DAG_STATE_FILENAME), "utf8"));
  assert.equal(stored.revision, 2);
  assert.deepEqual(stored.activeEdges, result.state.activeEdges);
  assert.deepEqual(stored.receipts.map(({ mutationId, revision }) => [mutationId, revision]), [
    ["setup", 1],
    ["insert", 2],
  ]);
}));

test("an exact insertion retry is idempotent and changed payload reuse conflicts", (t) => withStore(t, async (agentDir) => {
  await mutateSessionDagState(add("setup", 0, "edge-ab"), {
    agentDir,
    availableSessionIds: AVAILABLE,
  });
  const request = insert("stable-insert", 1);
  const accepted = await mutateSessionDagState(request, { agentDir, availableSessionIds: AVAILABLE });
  ...
  await assert.rejects(
    mutateSessionDagState(insert("stable-insert", 1, "edge-ab", "D", "edge-ad", "edge-db"), {
      agentDir,
      availableSessionIds: AVAILABLE,
    }),
...
test("add, replace, and insert reject an invalidated complete session listing under the lock", (t) => withStore(t, async (agentDir) => {
  const originalGeneration = globalThis.__piSessionListGeneration;
  try {
    globalThis.__piSessionListGeneration = 9;
    const expected = { formId: "default", fromSessionId: "A", toSessionId: "B" };
    for (const request of [
      add("stale-add-list", 0, "edge-1"),
      mutation("stale-replace-list", 0, {
        type: "replace_edge",
        edgeId: "edge-ab",
        expected,
        next: { fromSessionId: "A", toSessionId: "C" },
      }),
      insert("stale-insert-list", 0),
    ]) {
      await assert.rejects(
        mutateSessionDagState(request, {
          agentDir,
          availableSessionIds: AVAILABLE,
...
test("add, replace, and insert recheck listing generation after asynchronous state reading", (t) => withStore(t, async (agentDir) => {
  const expected = { formId: "default", fromSessionId: "A", toSessionId: "B" };
  for (const request of [
    add("raced-add-list", 0, "edge-1"),
    mutation("raced-replace-list", 0, {
      type: "replace_edge",
      edgeId: "edge-ab",
      expected,
      next: { fromSessionId: "A", toSessionId: "C" },
    }),
    insert("raced-insert-list", 0),
  ]) {
...
test("rejected insertions preserve the original stored edge and exact serialized authority", async (t) => {
  const expectConflictWithoutWrite = async (agentDir, request, options, code) => {
    const path = join(agentDir, SESSION_DAG_STATE_FILENAME);
    const before = await readFile(path, "utf8");
    await assert.rejects(
      mutateSessionDagState(request, { agentDir, ...options }),
      (error) => error instanceof SessionDagMutationConflictResponseError
        && error.code === code,
    );
  ...
  await withStore(t, async (agentDir) => {
    ...
    await expectConflictWithoutWrite(
      agentDir,
      insert("endpoint", 1, "edge-ab", "A", "endpoint-first", "endpoint-second"),
      { availableSessionIds: AVAILABLE },
      "session_dag_insert_endpoint",
    );
    await expectConflictWithoutWrite(
      agentDir,
      insert("id-reuse", 1, "edge-ab", "C", "edge-ab", "fresh-second"),
      { availableSessionIds: AVAILABLE },
      "session_dag_target_changed",
    );
    await expectConflictWithoutWrite(
      agentDir,
      insert("unavailable", 1),
      { availableSessionIds: new Set(["A", "B"]) },
      "session_dag_session_not_found",
    );
    await mutateSessionDagState(add("existing-ac", 1, "edge-existing-ac", "A", "C"), {
      agentDir,
      availableSessionIds: AVAILABLE,
    });
    await expectConflictWithoutWrite(
  ...
  await withStore(t, async (agentDir) => {
    await mutateSessionDagState(add("add-cd", 0, "edge-cd", "C", "D"), {
      agentDir,
      availableSessionIds: AVAILABLE,
    });
    await mutateSessionDagState(complete("complete-c", 1, "batch-c", "C", ["edge-cd"]), { agentDir });
    await mutateSessionDagState(add("add-ab", 2, "edge-ab"), {
      agentDir,
      availableSessionIds: AVAILABLE,
    });
    await expectConflictWithoutWrite(
  ...
  await withStore(t, async (agentDir) => {
    ...
    const counterState = {
      ...
      nextEdgeOrder: Number.MAX_SAFE_INTEGER,
    };
    await writeFile(path, `${JSON.stringify(counterState, null, 2)}\n`, { mode: 0o600 });
    await expectConflictWithoutWrite(
      agentDir,
      insert("counter", 0),
      { availableSessionIds: AVAILABLE },
    ...
    await writeFile(path, `${JSON.stringify(capacityState, null, 2)}\n`, { mode: 0o600 });
    await expectConflictWithoutWrite(
      agentDir,
      insert("capacity", 0),
      { availableSessionIds: AVAILABLE },
...
test("canonical mutation digests ignore object key order but retain semantic differences", () => {
  ...
  assert.notEqual(
    digestSessionDagMutation(insert("insert-digest", 1)),
    digestSessionDagMutation(insert("insert-digest", 1, "edge-ab", "D", "edge-ad", "edge-db")),
  );
});
