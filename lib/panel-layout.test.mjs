...
test("keeps an unset right panel responsive at the current 42 percent default", async () => {
  const {
...
test("geometry-derived handle range follows each split-visible panel across compact widths", async () => {
  const {
    PANEL_LAYOUT_MOBILE_MAX_WIDTH,
    resolveEffectivePanelLayout,
  } = await loadSubject();
  const expectedEligibility = new Map([
    [640, { sidebarOnly: false, rightOnly: false, both: { sidebar: false, rightPanel: false } }],
    [641, { sidebarOnly: true, rightOnly: true, both: { sidebar: false, rightPanel: false } }],
    [768, { sidebarOnly: true, rightOnly: true, both: { sidebar: false, rightPanel: false } }],
    [800, { sidebarOnly: true, rightOnly: true, both: { sidebar: false, rightPanel: false } }],
    [900, { sidebarOnly: true, rightOnly: true, both: { sidebar: true, rightPanel: true } }],
    [999, { sidebarOnly: true, rightOnly: true, both: { sidebar: true, rightPanel: true } }],
    [1_000, { sidebarOnly: true, rightOnly: true, both: { sidebar: true, rightPanel: true } }],
  ]);

  for (const [viewportWidth, expected] of expectedEligibility) {
    const splitVisible = viewportWidth > PANEL_LAYOUT_MOBILE_MAX_WIDTH;
    const resolve = (sidebarOpen, rightPanelOpen) => resolveEffectivePanelLayout({
      viewportWidth,
      sidebarVisible: splitVisible && sidebarOpen,
      rightPanelVisible: splitVisible && rightPanelOpen,
      sidebarPreferredWidth: null,
      rightPanelPreferredWidth: null,
    });
    const sidebarOnly = resolve(true, false);
    const rightOnly = resolve(false, true);
    const both = resolve(true, true);

    assert.equal(
      splitVisible && sidebarOnly.sidebarBounds.maxWidth > sidebarOnly.sidebarBounds.minWidth,
      expected.sidebarOnly,
      `sidebar-only eligibility at ${viewportWidth}px`,
    );
    assert.equal(
      splitVisible && rightOnly.rightPanelBounds.maxWidth > rightOnly.rightPanelBounds.minWidth,
      expected.rightOnly,
      `right-only eligibility at ${viewportWidth}px`,
    );
    assert.equal(
      splitVisible && both.sidebarBounds.maxWidth > both.sidebarBounds.minWidth,
      expected.both.sidebar,
      `two-panel sidebar eligibility at ${viewportWidth}px`,
    );
    assert.equal(
      splitVisible && both.rightPanelBounds.maxWidth > both.rightPanelBounds.minWidth,
      expected.both.rightPanel,
      `two-panel right-panel eligibility at ${viewportWidth}px`,
    );
  }
});

test("a visible fixed panel still constrains the other boundary and shrinking can transfer range", async () => {
  const {
    getRightPanelWidthBounds,
    getSidebarWidthBounds,
    resolveEffectivePanelLayout,
  } = await loadSubject();
  const initial = resolveEffectivePanelLayout({
    viewportWidth: 801,
    sidebarVisible: true,
    rightPanelVisible: true,
    sidebarPreferredWidth: null,
  ...
  assert.deepEqual({
    ...
    sidebarBounds: initial.sidebarBounds,
    rightPanelWidth: initial.rightPanelWidth,
    rightPanelBounds: initial.rightPanelBounds,
  }, {
    sidebarWidth: 181,
    sidebarBounds: { minWidth: 180, maxWidth: 181 },
    rightPanelWidth: 300,
    rightPanelBounds: { minWidth: 300, maxWidth: 300 },
  });

  const sidebarBoundsAfterShrink = getSidebarWidthBounds({
    viewportWidth: 801,
    rightPanelVisible: true,
    rightPanelWidth: initial.rightPanelWidth,
  });
  const rightPanelBoundsAfterShrink = getRightPanelWidthBounds({
    viewportWidth: 801,
    sidebarVisible: true,
    sidebarWidth: 180,
  });
  assert.deepEqual({
    sidebarBoundsAfterShrink,
    rightPanelBoundsAfterShrink,
  }, {
    sidebarBoundsAfterShrink: { minWidth: 180, maxWidth: 181 },
    rightPanelBoundsAfterShrink: { minWidth: 300, maxWidth: 301 },
  });
});
...
test("AppShell and the shared hook preserve accessible mounted split behavior", async () => {
  ...
  assert.match(shell, /aria-controls="right-panel"/);
  assert.match(shell, /id="right-panel"/);
  assert.doesNotMatch(shell, /PANEL_RESIZE_MIN_VIEWPORT_WIDTH|panelResizeEligible/);
  assert.match(shell, /const splitLayoutVisible = !isMobile && !fileViewerExpandedActive/);
  assert.match(shell, /const sidebarSplitVisible = splitLayoutVisible && sidebarOpen/);
  assert.match(shell, /const rightPanelSplitVisible = splitLayoutVisible && rightPanelOpen/);
  assert.match(shell, /rightPanelVisible: rightPanelSplitVisible/);
  assert.match(shell, /sidebarVisible: sidebarSplitVisible/);
  assert.match(shell, /const sidebarResizeEnabled = sidebarSplitVisible\s*&& sidebarResizeBounds\.maxWidth > sidebarResizeBounds\.minWidth/);
  assert.match(shell, /const rightPanelResizeEnabled = rightPanelSplitVisible\s*&& rightPanelResizeBounds\.maxWidth > rightPanelResizeBounds\.minWidth/);
  ...
  const panelEligibility = shell.slice(
    shell.indexOf("const splitLayoutVisible"),
    shell.indexOf("const getResponsiveRightPanelWidth"),
  );
  assert.doesNotMatch(panelEligibility, /activeRightPanelTabId|fileTabs|FileViewer|SessionDagPanel/);
  assert.doesNotMatch(panelEligibility, /rightPanelVisible: rightPanelResizeEnabled/);
  assert.doesNotMatch(panelEligibility, /sidebarVisible: sidebarResizeEnabled/);
  ...
  assert.doesNotMatch(css, /@media \(max-width: 999px\)[\s\S]*panel-resize-handle/);
  assert.match(css, /panel-resize-handle\s*\{[\s\S]*touch-action: none/);
  assert.match(css, /panel-resize-handle::after\s*\{[\s\S]*width: 2px/);
  assert.match(css, /@media \(pointer: coarse\), \(any-pointer: coarse\)\s*\{[\s\S]*panel-resize-handle\s*\{[\s\S]*width: 24px[\s\S]*margin-inline: -12px[\s\S]*flex-basis: 24px/);
  assert.match(css, /@media \(max-width: 640px\)[\s\S]*panel-resize-handle[\s\S]*display: none !important/);
