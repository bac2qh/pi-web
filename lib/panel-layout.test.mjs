...
test("AppShell and the shared hook preserve accessible mounted split behavior", async () => {
  ...
  assert.match(shell, /\{dagActivated && <SessionDagPanel active=\{dagPanelActive\} \/>\}/);
  const rightPanelEligibility = shell.slice(
    shell.indexOf("const rightPanelResizeEnabled"),
    shell.indexOf("const sidebarWidthRef"),
  );
  assert.doesNotMatch(rightPanelEligibility, /activeRightPanelTabId|fileTabs|FileViewer|SessionDagPanel/);
