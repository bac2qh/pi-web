...
test("preserves optional ticket context by identity and deletes it on every ticket terminal path", () => {
  const clock = createFakeClock();
  const gateway = createTestGateway({ clock });
  assert.equal(gateway.ticketContextVersion, 1);
  const handler = () => {};
  const unregister = gateway.registerChannel("test.context", handler);
  const context = Object.freeze({ opaque: true });
  const consumedTicket = gateway.issueTicket("test.context", context);
  const consumed = gateway.consumeTicket(consumedTicket.ticket);
  assert.strictEqual(consumed.ticketContext, context);
  assert.equal(gateway.getStats().pendingTicketCount, 0);
  assert.equal(gatewayErrorCode(() => gateway.consumeTicket(consumedTicket.ticket)), "invalid_ticket");

  const expired = gateway.issueTicket("test.context", context);
  clock.advance(PI_WEB_TRANSPORT_TICKET_TTL_MS);
  assert.equal(gatewayErrorCode(() => gateway.consumeTicket(expired.ticket)), "invalid_ticket");
  const revoked = gateway.issueTicket("test.context", context);
  unregister();
  assert.equal(gatewayErrorCode(() => gateway.consumeTicket(revoked.ticket)), "invalid_ticket");

  gateway.registerChannel("test.context", handler);
  const closed = gateway.issueTicket("test.context", context);
  gateway.close();
  assert.equal(gatewayErrorCode(() => gateway.consumeTicket(closed.ticket)), "gateway_closed");
});
...
test("mixed running/session/file-watch tickets consume before deterministic 256/257 admission and restore capacity", () => {
  const gateway = createTestGateway();
  gateway.registerChannel("running", () => {});
  gateway.registerChannel("session", () => {});
  gateway.registerChannel("file-watch", () => {});
  const releases = [];
  const channels = ["running", "session", "file-watch"];
  for (let index = 0; index < PI_WEB_TRANSPORT_MAX_CONNECTIONS_TOTAL; index += 1) {
    const channel = channels[index % channels.length];
    const issued = gateway.issueTicket(channel, channel === "running" ? undefined : Object.freeze({ opaque: true }));
    const consumed = gateway.consumeTicket(issued.ticket);
    assert.equal(consumed.channel, channel);
    releases.push(gateway.reserveConnection(`peer-${index % 4}`));
  }
  const rejectedTicket = gateway.issueTicket("file-watch", Object.freeze({ opaque: true }));
  gateway.consumeTicket(rejectedTicket.ticket);
  assert.equal(gatewayErrorCode(() => gateway.reserveConnection("peer-fifth")), "connection_limit");
  assert.equal(gatewayErrorCode(() => gateway.consumeTicket(rejectedTicket.ticket)), "invalid_ticket");
  releases[0]();
  const readmitted = gateway.reserveConnection("peer-fifth");
  assert.equal(gateway.getStats().activeConnectionCount, 256);
  readmitted();
