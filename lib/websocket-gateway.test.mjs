...
test("mixed running/session tickets consume before deterministic 256/257 admission and restore capacity", () => {
  ...
  for (let index = 0; index < PI_WEB_TRANSPORT_MAX_CONNECTIONS_TOTAL; index += 1) {
    const channel = index % 2 === 0 ? "running" : "session";
    const issued = gateway.issueTicket(channel, channel === "session" ? Object.freeze({ opaque: true }) : undefined);
  ...
  const rejectedTicket = gateway.issueTicket("session", Object.freeze({ opaque: true }));
