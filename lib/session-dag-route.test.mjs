...
function handlers(overrides = {}) {
  return createSessionDagRouteHandlers({
    ...
    listSessions: async () => ({ sessions: [], generation: 0 }),
    ...overrides,
  });
}
...
test("GET is no-store and never lists or reconciles native sessions", async () => {
  let listCalls = 0;
  const route = handlers({ listSessions: async () => { listCalls += 1; throw new Error("must not list"); } });
...
test("add and replace use a complete generation-current listing and retry only its bounded race", async () => {
  let listCalls = 0;
  ...
  assert.deepEqual([...mutationOptions[1].availableSessionIds], ["A", "B"]);
...
test("completion, history, form, and direction operations never require current session discovery", async () => {
  ...
  let mutateCalls = 0;
  const route = handlers({
    listSessions: async () => { listCalls += 1; throw new Error("unexpected list"); },
    mutateState: async (_envelope, options) => {
      mutateCalls += 1;
...
test("conflicts return 409 authority while listing failures and unknown failures are sanitized", async () => {
  ...
  assert.deepEqual(await conflictResponse.json(), {
    error: "Graph changed elsewhere; review and retry",
    code: "session_dag_revision_conflict",
    state: authoritative,
  });

  const listingRoute = handlers({ listSessions: async () => { throw new Error("private session path"); } });
