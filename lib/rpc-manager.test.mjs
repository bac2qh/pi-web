...
const {
  ...
  resolveExistingMessageFreeThinkingLevel,
  startRpcSession,
...
const EFFORT_FIXTURE_PROVIDER = "pi-web-effort-fixture";
const EFFORT_FIXTURE_MODEL = "xhigh-model";
const EFFORT_FIXTURE_COMMAND = "hosted-effort-fixture";

function createEffortStartupFixture(t) {
  const root = mkdtempSync(join(tmpdir(), "pi-web-rpc-effort-"));
  const agentDir = join(root, "agent");
  const sessionDir = join(root, "sessions");
  mkdirSync(agentDir, { recursive: true });

  const settingsText = `${JSON.stringify({
    defaultProvider: EFFORT_FIXTURE_PROVIDER,
    defaultModel: EFFORT_FIXTURE_MODEL,
    defaultThinkingLevel: "max",
  }, null, 2)}\n`;
  writeFileSync(join(agentDir, "settings.json"), settingsText);
  writeFileSync(join(agentDir, "auth.json"), "{}\n");
  writeFileSync(join(agentDir, "models.json"), `${JSON.stringify({
    providers: {
      [EFFORT_FIXTURE_PROVIDER]: {
        baseUrl: "https://fixture.invalid/v1",
        apiKey: "test-key",
        api: "openai-completions",
        models: [{
          id: EFFORT_FIXTURE_MODEL,
          name: "Xhigh Fixture Model",
          reasoning: true,
          input: ["text"],
  ...
  const fixture = {
    ...
    settingsText,
    createTarget({ thinkingLevels = [], populated = false, hostedCommand = false } = {}) {
      targetCount += 1;
      const cwd = join(root, `cwd-${targetCount}`);
      mkdirSync(cwd, { recursive: true });
      let markerPath;
      if (hostedCommand) {
        markerPath = join(root, `hosted-marker-${targetCount}.txt`);
        const extensionDir = join(cwd, ".pi", "extensions");
        mkdirSync(extensionDir, { recursive: true });
        writeFileSync(join(extensionDir, "hosted-effort-fixture.ts"), [
          'import { appendFileSync } from "node:fs";',
          "export default function (pi: any) {",
          `  pi.registerCommand(${JSON.stringify(EFFORT_FIXTURE_COMMAND)}, {`,
          '    description: "Record hosted startup effort",',
          "    async handler() {",
          `      appendFileSync(${JSON.stringify(markerPath)}, String(pi.getThinkingLevel()) + "\\n");`,
          "    },",
          "  });",
          "}",
          "",
      ...
      manager.appendSessionInfo(`effort-target-${targetCount}`);
      for (const level of thinkingLevels) manager.appendThinkingLevelChange(level);
      if (populated) manager.appendMessage(userMessage("existing conversation"));
      writeFileSync(
    ...
    async start(target, hooks) {
      ...
      activeSessions.add(result.session);
      return result;
    },
    track(session) {
    ...
    markerLevels(target) {
      if (!target.markerPath || !existsSync(target.markerPath)) return [];
      return readFileSync(target.markerPath, "utf8").trim().split("\n").filter(Boolean);
    },
    async cleanup() {
      if (cleaned) return;
      cleaned = true;
      await Promise.allSettled([...activeSessions].map((session) => session.shutdown()));
...
test("message-free existing effort resolver follows active-path native semantics", (t) => {
  const root = mkdtempSync(join(tmpdir(), "pi-web-rpc-effort-resolver-"));
  t.after(() => rmSync(root, { recursive: true, force: true }));
  let index = 0;
  const createManager = (configure) => {
    index += 1;
    const manager = SessionManager.create(join(root, `cwd-${index}`), join(root, "sessions"));
    const rootId = manager.appendSessionInfo(`resolver-${index}`);
    configure(manager, rootId);
    return manager;
  };

  const activeLastValue = createManager((manager) => {
    manager.appendThinkingLevelChange("low");
    manager.appendThinkingLevelChange("xhigh");
  });
  assert.equal(resolveExistingMessageFreeThinkingLevel(activeLastValue), "xhigh");

  const abandonedValue = createManager((manager, rootId) => {
    manager.appendThinkingLevelChange("max");
    manager.branch(rootId);
    manager.appendThinkingLevelChange("low");
  });
  assert.equal(abandonedValue.getEntries().some((entry) => (
    entry.type === "thinking_level_change" && entry.thinkingLevel === "max"
  )), true);
  assert.equal(abandonedValue.getBranch().some((entry) => (
    entry.type === "thinking_level_change" && entry.thinkingLevel === "max"
  )), false);
  assert.equal(resolveExistingMessageFreeThinkingLevel(abandonedValue), "low");

  const explicitOff = createManager((manager) => manager.appendThinkingLevelChange("off"));
  assert.equal(resolveExistingMessageFreeThinkingLevel(explicitOff), "off");

  const absent = createManager(() => {});
  assert.equal(absent.buildSessionContext().thinkingLevel, "off", "native absence defaults context to off");
  assert.equal(resolveExistingMessageFreeThinkingLevel(absent), undefined);

  const malformedFinal = createManager((manager) => {
    manager.appendThinkingLevelChange("xhigh");
    manager.appendThinkingLevelChange("ultra");
  });
  assert.equal(malformedFinal.buildSessionContext().thinkingLevel, "ultra");
  assert.equal(resolveExistingMessageFreeThinkingLevel(malformedFinal), undefined);

  const populated = createManager((manager) => {
    manager.appendThinkingLevelChange("xhigh");
    manager.appendMessage(userMessage("existing conversation"));
  });
  assert.equal(populated.buildSessionContext().messages.length, 1);
  assert.equal(resolveExistingMessageFreeThinkingLevel(populated), undefined);
});

test("valid message-free existing effort is effective before publication without changing settings", { timeout: 30_000 }, async (t) => {
  const fixture = createEffortStartupFixture(t);
  const target = fixture.createTarget({ thinkingLevels: ["xhigh"] });
  const initialEntries = target.manager.getEntries();
  const observations = [];

  const result = await fixture.start(target, {
    validatePrepared({ session }) {
      observations.push({ stage: "prepared", session });
      assert.equal(session.inner.agent.state.thinkingLevel, "xhigh");
      assert.equal(getRpcSession(target.sessionId), undefined);
    },
    afterPublication({ session }) {
      observations.push({ stage: "published", session });
      assert.equal(session.inner.agent.state.thinkingLevel, "xhigh");
      assert.strictEqual(getRpcSession(target.sessionId), session);
    },
  });

  assert.deepEqual(observations.map(({ stage }) => stage), ["prepared", "published"]);
  assert.ok(observations.every(({ session }) => session === result.session));
  assert.equal(result.session.inner.model.provider, EFFORT_FIXTURE_PROVIDER);
  assert.equal(result.session.inner.model.id, EFFORT_FIXTURE_MODEL);
  assert.equal(result.session.inner.model.thinkingLevelMap.xhigh, "xhigh");
  assert.equal(result.session.inner.agent.state.messages.length, 0);
  assert.equal(result.session.inner.agent.state.thinkingLevel, "xhigh");

  const entries = result.session.inner.sessionManager.getEntries();
  const appended = entries.slice(initialEntries.length);
  assert.deepEqual(initialEntries.map((entry) => entry.type), ["session_info", "thinking_level_change"]);
  assert.deepEqual(appended.map((entry) => entry.type), ["model_change", "thinking_level_change"]);
  assert.deepEqual(
    { provider: appended[0].provider, modelId: appended[0].modelId },
    { provider: EFFORT_FIXTURE_PROVIDER, modelId: EFFORT_FIXTURE_MODEL },
  );
  assert.equal(appended[1].thinkingLevel, "xhigh");
  assert.equal(entries.some((entry) => entry.type === "custom"), false, "Pi Web adds no startup entry");
  assert.equal(readFileSync(fixture.settingsPath, "utf8"), fixture.settingsText);
  await fixture.cleanup();
});

test("absent, malformed, new, and populated session startup retain native behavior", { timeout: 60_000 }, async (t) => {
  const fixture = createEffortStartupFixture(t);
  const messageFreeCases = [
    { name: "explicit off", thinkingLevels: ["off"], resolved: "off", effective: "off" },
    { name: "absent", thinkingLevels: [], resolved: undefined, effective: "max" },
    { name: "malformed final", thinkingLevels: ["xhigh", "ultra"], resolved: undefined, effective: "max" },
  ];

  for (const testCase of messageFreeCases) {
    const target = fixture.createTarget({ thinkingLevels: testCase.thinkingLevels });
    const initialEntryCount = target.manager.getEntries().length;
    assert.equal(resolveExistingMessageFreeThinkingLevel(target.manager), testCase.resolved, testCase.name);
    const result = await fixture.start(target, {
      validatePrepared({ session }) {
        assert.equal(session.inner.agent.state.thinkingLevel, testCase.effective, testCase.name);
      },
    });
    const entries = result.session.inner.sessionManager.getEntries();
    assert.deepEqual(
      entries.slice(initialEntryCount).map((entry) => entry.type),
      ["model_change", "thinking_level_change"],
      testCase.name,
    );
    assert.equal(entries.at(-1).thinkingLevel, testCase.effective, testCase.name);
    await result.session.shutdown();
  }

  const populated = fixture.createTarget({ thinkingLevels: ["xhigh"], populated: true });
  const populatedEntryCount = populated.manager.getEntries().length;
  assert.equal(resolveExistingMessageFreeThinkingLevel(populated.manager), undefined);
  const populatedResult = await fixture.start(populated, {
    validatePrepared({ session }) {
      assert.equal(session.inner.agent.state.thinkingLevel, "xhigh");
      assert.equal(session.inner.agent.state.messages.length, 1);
    },
  });
  assert.equal(
    populatedResult.session.inner.sessionManager.getEntries().length,
    populatedEntryCount,
    "populated native restoration appends no initialization entry when thinking is already declared",
  );
  await populatedResult.session.shutdown();

  const newCwd = join(fixture.root, "new-session-cwd");
  mkdirSync(newCwd, { recursive: true });
  const newResult = await startRpcSession(`new-effort-session-${Date.now()}`, "", newCwd);
  fixture.track(newResult.session);
  assert.equal(newResult.session.inner.agent.state.thinkingLevel, "max");
  assert.deepEqual(
    newResult.session.inner.sessionManager.getEntries().map((entry) => entry.type),
    ["model_change", "thinking_level_change"],
  );
  assert.equal(newResult.session.inner.sessionManager.getEntries().at(-1).thinkingLevel, "max");
  assert.equal(readFileSync(fixture.settingsPath, "utf8"), fixture.settingsText);
  await fixture.cleanup();
});

test("hosted-first and browser-first startup share one xhigh owner and one kickoff", { timeout: 60_000 }, async (t) => {
  const fixture = createEffortStartupFixture(t);
  const capability = globalThis[HOSTED_IMPLEMENTATION_CAPABILITY_SYMBOL];
  assert.ok(capability?.active);

  for (const order of ["hosted-first", "browser-first"]) {
    const target = fixture.createTarget({ thinkingLevels: ["xhigh"], hostedCommand: true });
    assert.equal(resolveExistingMessageFreeThinkingLevel(target.manager), "xhigh");
    const request = {
      targetSessionId: target.sessionId,
      targetSessionFile: target.sessionFile,
      targetCwd: target.cwd,
      kickoff: `/${EFFORT_FIXTURE_COMMAND}`,
      launchKind: "start",
    ...
    let launch;
    let browser;
    if (order === "hosted-first") {
      [launch, browser] = await Promise.all([
        capability.launch(request),
        fixture.start(target),
      ]);
    } else {
      browser = await fixture.start(target);
      launch = await capability.launch(request);
    }

    const owner = fixture.track(getRpcSession(target.sessionId));
    assert.ok(owner, order);
    assert.strictEqual(browser.session, owner, order);
    assert.strictEqual((await fixture.start(target)).session, owner, order);
    assert.equal(launch.targetSessionId, target.sessionId, order);
    assert.equal(owner.inner.agent.state.thinkingLevel, "xhigh", order);

    await waitForCondition(
      () => fixture.markerLevels(target).length === 1 && !owner.isRunning(),
      `${order} hosted kickoff did not settle exactly once`,
    );
    await new Promise((resolve) => setImmediate(resolve));
    assert.deepEqual(fixture.markerLevels(target), ["xhigh"], order);
    assert.equal(
      owner.inner.sessionManager.getEntries().some((entry) => entry.type === "message"),
      false,
      `${order} fixture command must not dispatch a provider turn`,
    );
    assert.equal(readFileSync(fixture.settingsPath, "utf8"), fixture.settingsText, order);
    await owner.shutdown();
    assert.equal(getRpcSession(target.sessionId), undefined, order);
  }
...
test("semantic idle uses the exact 12-hour default and an injected touch boundary", async (t) => {
  assert.equal(RPC_SESSION_IDLE_TIMEOUT_MS, 12 * 60 * 60 * 1000);
  const { manager } = createSource(t);
  const state = {};
