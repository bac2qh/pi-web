...
test("a delayed committed terminal publishes idle only after projected finality", (t) => {
  resetRunningProjectionState();
  const { manager } = createSource(t);
  let nativeListener;
  const wrapper = new AgentSessionWrapper(fakeInner(manager, {
  ...
  assert.equal(wrapper.isRunning(), true, "the reserved terminal remains active while its receipt is queued");
  assert.deepEqual(getRunningRpcSessionIds(), [wrapper.sessionId]);
