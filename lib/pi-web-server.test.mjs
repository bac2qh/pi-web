  ...
  const runCase = async ({ handler, handshakeThrows = false }) => {
    ...
    const upgrade = createPiWebUpgradeHandler({
      gateway: {
        serverInstanceId: "test-instance",
        isSameHostOrigin: () => true,
        consumeTicket: () => ({ channel: "running", handler }),
        reserveConnection: () => {
          let released = false;
          return () => {
            if (released) return false;
...
test("real malformed handshake releases admission and permits a valid re-admission", {
  ...
  let handlerCalls = 0;
  server.gateway.registerChannel("test.handshake", () => { handlerCalls += 1; });
...
test("real Node loopback sockets enforce per-peer admission with release and re-admission", {
  timeout: 60_000,
}, async (t) => {
  const nextFactory = () => ({
    async prepare() {},
  ...
  t.after(() => server.close().catch(() => {}));
  let handlerCalls = 0;
  server.gateway.registerChannel("test.admission", () => { handlerCalls += 1; });
  const origin = `http://127.0.0.1:${server.address.port}`;
  const base = `ws://127.0.0.1:${server.address.port}${PI_WEB_TRANSPORT_PATH}`;

  const open = async () => {
    const { ticket } = server.gateway.issueTicket("test.admission");
    return new Promise((resolve, reject) => {
