...
test("recognizes only exact completed write calls without treating them as edits", async () => {
  const { containsCompletedWriteToolCall, isEditToolName, isWriteToolName } = await loadSubject();
  const unrecognized = ["Write", "WRITE", "writer", "write_file", "workspace.write", "prewrite"];

  assert.equal(isWriteToolName("write"), true);
  assert.equal(isEditToolName("write"), false, "write keeps a rendering path separate from edit patches");
  for (const name of unrecognized) assert.equal(isWriteToolName(name), false, name);

  const blocks = [
    { type: "toolCall", toolCallId: "write-one", toolName: "write", input: {} },
    { type: "toolCall", toolCallId: "near", toolName: "workspace.write", input: {} },
  ];
  assert.equal(containsCompletedWriteToolCall(blocks, new Set()), false);
  assert.equal(containsCompletedWriteToolCall(blocks, new Set(["orphan"])), false);
  assert.equal(containsCompletedWriteToolCall(blocks, new Set(["near"])), false);
  assert.equal(containsCompletedWriteToolCall(blocks, new Set(["write-one"])), true);
});
