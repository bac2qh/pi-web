...
test("final-tab close clears both expansion sources and suppression", async () => {
  const { INITIAL_FILE_VIEWER_EXPANSION, fileViewerExpansionAfterFinalClose } = await loadSubject();

  assert.deepEqual(fileViewerExpansionAfterFinalClose(), INITIAL_FILE_VIEWER_EXPANSION);
});
