...
const { POST, createFileWatchTicketIssuer, createSessionTicketIssuer } = await jiti.import("../app/api/transport/ticket/route.ts");
...
test("file-watch issuer rejects denied, missing, directory, and symlink-classifies without session startup", async () => {
  ...
  const routeSource = await readFile(new URL("../app/api/transport/ticket/route.ts", import.meta.url), "utf8");
  const fileIssuerSource = routeSource.slice(routeSource.indexOf("export function createFileWatchTicketIssuer"), routeSource.indexOf("const issueFileWatchTicket"));
