...
test("bufferedAmount fails closed and close fallback terminates at exactly 1000 ms", () => {
  ...
  const timers = [];
  const subscriptionOwner = { active: true, subscribers: new Set(), closeFallbacks: new Set() };
  const socket = new FakeSocket();
  ...
  assert.equal(subscriptionOwner.closeFallbacks.size, 1);
  timers[0].callback();
  assert.equal(fallback.socket.terminateCalls, 1);
  assert.equal(subscriptionOwner.closeFallbacks.size, 0, "fired fallback releases its owner-wide closure");
  assert.equal(subscriptionOwner.subscribers.size, 0);
});
...
test("active session policy fallback is cancelled on HMR and retired gateway force is suppressed by shutdown", () => {
  let now = 0;
  ...
  channel.ensureSessionChannel(oldGateway);
  const authorization = oldGateway.consumeTicket(oldGateway.issueTicket("session").ticket);
  const hub = new ProjectedSessionEventHub({ streamEpoch: "hmr-policy" });
  const wrapper = new FakeWrapper(hub);
  const socket = new FakeSocket();
  socket.close = function close(code, reason) { this.closeCalls.push([code, reason]); this.readyState = 2; };
  const enlisted = authorization.enlistSocket(socket);
  authorization.handler(socket, {
    ...context(wrapper, hub, oldGateway.serverInstanceId), ownerToken: enlisted.ownerToken,
  });
  socket.clientMessage("{}");
  const oldRecord = globalThis[channel.SESSION_REGISTRATION_TEST_SYMBOL];
  assert.equal(oldRecord.subscriptionOwner.closeFallbacks.size, 1);

  const replacementGateway = createPiWebTransportGateway();
  channel.ensureSessionChannel(replacementGateway);
  ...
  assert.equal(oldGateway.getOwnerLifecycleStats().closeFallbackCount, 0);
  advance(9_999);
  assert.equal(socket.terminateCalls, 0);
  socket.readyState = 3;
  socket.emit("close", 1000);
  assert.equal(oldGateway.getOwnerLifecycleStats().retiredRegistrationCount, 0);
  oldGateway.close();
