...
test("builds trusted namespaced Swap controls with endpoint labels and self-edge disabling", () => {
  const document = createFakeSvgDocument();
  const active = createSessionDagSwapControl(document, "Repo · From", "Repo · To", false);
  assert.equal(active.namespaceURI, "http://www.w3.org/2000/svg");
  assert.equal(active.localName, "g");
  assert.equal(active.getAttribute("class"), "session-dag-swap-control");
  assert.equal(active.getAttribute("role"), "button");
  assert.equal(active.getAttribute("tabindex"), "0");
  assert.equal(active.getAttribute("aria-label"), "Swap dependency from Repo · From to Repo · To");
  assert.equal(active.getAttribute("aria-disabled"), null);
  assert.deepEqual(active.children.map((child) => child.localName), ["rect", "text"]);
  assert.equal(active.children[1].textContent, "Swap");

  const disabled = createSessionDagSwapControl(document, "Same", "Same", true);
