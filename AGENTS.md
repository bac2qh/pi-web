...
- Ordinary launch defaults to literal `127.0.0.1:30141` and never requires Tailscale. Explicit `--tailscale-serve` keeps the backend on exact loopback and starts the ordinary foreground command for same-port Tailnet HTTPS/WSS; a custom port changes both sides, while Serve rejects `0`, `443`, and non-loopback hostnames.
- On Unix-like systems the direct command and its descendants share one private process group; Windows retains direct-child signaling. The launcher recognizes only the foreground CLI readiness marker, suppresses child output, keeps its streams attached, and never calls `unref()` on it.
- Intentional shutdown signals only the still-owned group or child, waits ten seconds, sends one `SIGKILL` if still safe, and waits ten more seconds for `close`. Unconfirmed cleanup rejects embedded close and makes only the executed terminal CLI exit `1`. A ready-command exit warns once and leaves loopback Pi Web running without retry. The launcher never inspects or rewrites shared Serve state; hard kills, power loss, and surviving orphans remain operator-managed limitations.
...
- Shared existing-file startup passes a valid final active-path native thinking declaration (`off | minimal | low | medium | high | xhigh | max`) into SDK construction only when the native context has no messages. This happens before publication, so hosted-first and browser-first opens share one effort-initialized owner; absent or malformed declarations, new and populated sessions, global defaults, and the SDK's standard message-free initialization entries retain their existing behavior.
...
- Add/replace proves both endpoint IDs exist in one fresh generation-current exact-ID listing under the graph lock. Atomic edge insertion proves the selected endpoints and inserted third session exist in that same current listing. This authorization scan reads only bounded native session headers from the standard session directories and never loads transcript bodies or project/worktree metadata. Accepted IDs remain durable if their session later disappears; unavailable nodes can still be completed, undone/redone, copied, or deleted. Sidebar Hidden has no graph meaning.
...
- PATCH envelopes carry a stable mutation ID, base revision, and compare-and-set targets. Exact retries use bounded private receipts; mutation-ID reuse, stale revisions, changed targets, and capacity conflicts return authoritative `409` state without silent client replay. Only add/replace/insert consult session discovery.
...
- Raw structured forms are canonical; exact endpoint IDs stay editable and current project/worktree-qualified labels remain presentation-only beneath them. Committed **Swap** reverses the displayed pair through the existing exact-CAS `replace_edge`; trailing-draft Swap is local until Enter adds it. Mermaid is generated one-way with deterministic `nN` node and `eN` edge aliases. Preview XML-validates one current-render graph root, accessibility metadata, inert node aliases, exact edge paths, active-content safety, and geometry; Mermaid 11.15 self-edges are accepted only as their exact three generated node-alias segments and map back to the compiled edge. Completion controls, persistent top-left active-node **+** controls, available-node bottom-right **Go to session** controls, and enlarged collapsed midpoint edge-action dots live in a trusted sibling SVG layer; node geometry fails closed if any trusted control centers would be too close. Go-to resolves the exact node ID through current unfiltered session metadata and invokes the existing application selection owner without fetching or mutating the DAG, changing shared sidebar metadata, rerendering Mermaid, or changing right-panel state; the sidebar retains its explicit-open effects for unread clearing and effective-worktree restoration, hidden state is preserved, and unavailable nodes have no control. One authored interaction may be open across node and edge controls: an expanded edge dot exposes enlarged Swap/Insert buttons arranged horizontally in TD and vertically in LR to avoid adjacent nodes and node controls, while a node **+** exposes explicit Incoming/Outgoing exact-ID addition through the existing `add_edge` authority. Their bounded mini-forms share one trusted sibling HTML overlay in the same ShadowRoot; validated Mermaid CSS cannot select any of them, and Preview failure never removes Raw.
...
### Edit and write results in the transcript
- `isEditToolName()` in `lib/message-display.ts` is the single edit recognition policy. Recognized edit cards start expanded, and a settled turn containing one starts its existing **Process details** group expanded; both disclosures remain independent mounted browser state and unrelated tools retain their collapsed default.
- Exact, case-sensitive `write` remains separate from edit recognition. Its pending card starts collapsed and mounts no arguments; the first matching success or failure result opens an untouched card once, while any mounted user toggle and later result replacement remain authoritative. A matching completed write inside the exact settled group also starts **Process details** open.
- A completed write with string `path` and `content` shows the complete call-time content as neutral, naturally sized, soft-wrapped code with decorative line numbers. Success is **Written content**, failure is **Attempted content** with the error retained; bounded theme-aware syntax falls back to complete plaintext for unsupported or oversized content, and malformed input uses ordinary JSON/result fallback.
- Successful edit `details.patch`/`details.diff` is parsed without mutating source data into factual files, hunks, old/new line numbers, and exactly three adjacent context lines. Distant context becomes counted omission rows; malformed, unsupported, unsafe-coordinate, or over-budget patches fall back intact to plaintext.
- Structured edit patches are unified top-to-bottom review cards with removed rows before added rows, natural transcript height, soft wrapping, and no nested scroll. Syntax decoration uses bounded, separate full-hunk old/new projections so omitted context and replacement state are not guessed; only structurally one-to-one replacements receive intra-line emphasis.
...
- Outside the `<=640px` overlay/full-width presentation, each open, non-expanded split panel exposes a focusable pointer/keyboard separator only when its current bounds have real range. Bounds always account for every other split-visible panel, including one whose own handle has no range; the separate `<1000px` automatic right-panel expansion remains authoritative until **Restore** reveals a split.
- `useResizablePanel` mutates only `--sidebar-width` or `--right-panel-width` during pointer movement, then commits one React/browser-local preference update; resize cancellation must restore pointer capture, cursor, and selection state. Coarse or any-coarse pointers receive a net-zero `24px` transparent target around the unchanged `2px` visual line.
