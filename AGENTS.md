...
- Add/replace proves both endpoint IDs exist in one generation-current complete session listing under the graph lock. Atomic edge insertion proves the selected endpoints and inserted third session exist in that same current listing. Accepted IDs remain durable if their session later disappears; unavailable nodes can still be completed, undone/redone, copied, or deleted. Sidebar Hidden has no graph meaning.
